...
% plotting parameter: only use for testing
pl = 0;
...
str = ['IntervalPolya_',num2str(n),'_',num2str(m),'.mat'];

try
	load(str);
catch
	PolyaHessInterval_Res(n,m);
	load(str);
end

u0 = zeros(size(pts0,1),1);
...
% triangles near the ray [0,0]->[1,0]
...
% Lemma 4.7 in [2], computation of quantities involved
% is detailed below 
...
% upper bound for |b^tb|
bb     = M0_int*u_int(Inside);
normb  = norm(bb)^2;

% upper bound for |A(w)|
ubAw   = max(abs(mat(:)))+w*norm(bb,'inf');


sqrt5    = sqrt(intval('5'));

% upper bound for error U
ConstU   = intval(2/(sqrt5-1))*max(ubInvAw,ubAw*normb.sup);
...
% and enclosing with the residual error
...
aa = C_ext^2/4/sqrt(2)*(sqrt(lam1^2+lam1));
cc = GradUS0^2;
...
% Vector of coefficients before the norm of U0
f1sing       = zeros(n,1);
f1sing(1)    = 2/tan(theta);
f1sing(2)    = -1/tan(theta);
f1sing(end)  = -1/tan(theta);
% ===============================

% do we have something better for |f1-f1h|?
diffh1       = diffh;
%diffh1       = 2*sqrt(2)*sqrt(2/n)*Cgradu+1/tan(theta)*sqrt(2/n)*Cgradu
%diffh1       = 2/sin(theta)*sqrt(2/n)*Cgradu+4*sqrt(lam1)/(n*sin(theta)*sqrt(1+lam1))*(Cgradu+sqrt(lam1)*Cu);
% ==================================
...
% do we have something better for |f2-f2h|?
...
for k=1:n-1
	...
	% vector of coefficients for singular part
	falphasing       = -(1-cos(k*theta))*cos(theta)/sin(theta)*2*cos(jj*k*theta);
	...
	ia = midrad(a.mid,a.rad+erralpha.sup);
	ib = midrad(b.mid,b.rad+errbeta.sup);
	ic = midrad(c.mid,c.rad+errgamma1.sup);
